...
async function Task() {
  ...
  const localBundleVersion =
    (await AB.get('hitokoto:bundle:version')) || '0.0.0'
  const localBundleUpdatedAt = (await AB.get('hitokoto:bundle:updated_at')) || 0
  const localBundleVersionData =
    (await AB.get('hitokoto:bundle:version:record')) || {}
  // 获取远程数据的版控文件
  const remoteUrl =
  ...
  winston.verbose(
    '[sentencesUpdateTask] fetching version data from: ' + remoteVersionUrl,
  )
  ...
  if (!semver.satisfies(remoteVersionData.protocol_version, '>=1.0 <1.1')) {
    winston.error(
      '[sentencesUpdateTask] This program version is NOT support the protocol version: ' +
  ...
  if (
    ...
    localBundleUpdatedAt === remoteVersionData.updated_at
  ) {
    winston.verbose(
  ...
  let sentenceTotal = 0
  if (localBundleVersion === '0.0.0') {
    // 初次启动、全量同步数据
    // 获取分类数据
    const remoteCategoriesUrl = url.resolve(
      remoteUrl,
    ...
    for (const category of remoteCategoriesData) {
      ...
      // 保存句子长度范围
      await SideAB.set(
        `hitokoto:bundle:category:${category.key}:max`,
        maxLength,
      )
      await SideAB.set(
        `hitokoto:bundle:category:${category.key}:min`,
        minLength,
      )
    ...
    if (
      ...
      localBundleVersionData.categories.timestamp
    ) {
      // 读取远端分类信息
      const remoteCategoriesUrl = url.resolve(
        remoteUrl,
    ...
    for (const categoryVersion of localBundleVersionData.sentences) {
      ...
      if (remoteVersion.timestamp !== categoryVersion.timestamp) {
        ...
        for (const sentence of categorySentences) {
          ...
          queue.zadd([
            'hitokoto:bundle:category:' + categoryVersion.key,
            sentence.length,
            sentence.uuid,
          ])
        }
    ...
    for (const category of categories) {
      sentenceTotal += Number.parseInt(
        await SideAB.command('zcount', [
          'hitokoto:bundle:category:' + category.key,
          '0',
          '+inf',
        ]),
